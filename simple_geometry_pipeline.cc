...
#include "mediapipe/modules/face_geometry/libs/simple_geometry_pipeline.h"
...
class ScreenToMetricSpaceConverter {
 public:
  ...
  absl::Status Convert(const NormalizedLandmarkList& screen_landmark_list,  //
    ...
    LOG(INFO) << "Im converting simple geompetry";
    ...
    pose_transform_mat = Eigen::MatrixXf::Identity(4,4);
...
class SimpleGeometryPipelineImpl : public SimpleGeometryPipeline {
 public:
  SimpleGeometryPipelineImpl(
  ...
  absl::StatusOr<std::vector<FaceGeometry>> EstimateFaceGeometry(
    ...
    for (const NormalizedLandmarkList& screen_face_landmarks :
      ...
      const float f = 1.0f / std::tan(kDegreesToRadians *
                                      perspective_camera_.vertical_fov_degrees() / 2.f);

      W = (f / aspect_ratio) * selected_z;
      H = f * selected_z;

      LOG(INFO) << "aspect ratio is " << aspect_ratio << " w and h" << W << " " << H << " and f: " << f;
      ...
      for (int i = 0; i < canonical_mesh_num_vertices_; ++i) {
        ...
        mutable_mesh->set_vertex_buffer(vertex_buffer_offset,
                                        screen_face_landmarks.landmark(i).x() * W - W/2);
        mutable_mesh->set_vertex_buffer(vertex_buffer_offset + 1,
                                        screen_face_landmarks.landmark(i).y() * H - H/2);
        mutable_mesh->set_vertex_buffer(vertex_buffer_offset + 2, selected_z);
      }
...
absl::StatusOr<std::unique_ptr<SimpleGeometryPipeline>> CreateGeometryPipeline(
  ...
  // number of each vertex per point = 5
  uint32_t canonical_mesh_vertex_size =
      GetVertexSize(canonical_mesh.vertex_type());
  // number of all points = 468
  uint32_t canonical_mesh_num_vertices =
  ...
  // LOG(INFO) << "all vertex size " <<  canonical_mesh_vertex_size << "\n numer of vertices: " << canonical_mesh_num_vertices << "\n offset:" <<  canonical_mesh_vertex_position_offset;
  ...
  // add weights to landmarks
  for (const WeightedLandmarkRef& wlr : metadata.procrustes_landmark_basis()) {
    uint32_t landmark_id = wlr.landmark_id();
    landmark_weights(landmark_id) = wlr.weight() * 1000;
  }

  std::unique_ptr<SimpleGeometryPipeline> result =
      absl::make_unique<SimpleGeometryPipelineImpl>(
